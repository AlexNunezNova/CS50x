...
void blur(int height, int width, RGBTRIPLE image[height][width])
{
    ...
    for (int i = 0; i < height; i++)
    {
        ...
        for (int j = 0; j < width; j++)
        {
            ...
            //2.3 then, go pixel by pixel with an if statement to check if the pixel exists (8 ifs)
            ...
            copyimage[i][j].rgbtRed = round(sumRed / totalPixels);
            copyimage[i][j].rgbtBlue = round(sumBlue / totalPixels);
            copyimage[i][j].rgbtGreen = round(sumGreen / totalPixels);

            //2.5 check to limit RGB values to 255
    ...
    // 3. Copy to the definitive image
    for (int i = 0; i < height; i++)
    {
        for (int j = 0; j < width; j++)
        {
